...
class Train(object):
    ...
    def setup_train(self):
        ...
        # Clear the model for light save
        #save_path = config.save_model_path + "/"+self.opt.load_model+"_light.tar"
        #T.save({
        #    "iter": 15000,
        #    "model_dict": self.model.state_dict(),
        #}, save_path)    
    ...
    def train_batch_MLE(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, batch):
        ...
        for t in range(min(max_dec_len, config.max_dec_steps)):
            # --------- Indicator print
            self.counter_i += 1
            if self.counter_i < 3: self.timer_start=time.time()
            if self.counter_i % 50 == 49: sys.stdout.write('.'); sys.stdout.flush();
            if self.counter_i % 5000 == 0: print("timer: {:2.2}".format(time.time()-self.timer_start))
#            if self.counter_i % 5000 == 0: print()
            try:
                len_enc_out = (T.rand(len(enc_out)) > 0.25)
                use_gound_truth = get_cuda(len_enc_out).long()                        #Probabilities indicating whether to use ground truth labels instead of previous decoded tokens
            except:
                print(len_enc_out)
                print("CUDA exception, trying again")
                len_enc_out = (T.rand(len(enc_out)) > 0.5)
                use_gound_truth = get_cuda(len_enc_out).long()
    ...
    def trainIters(self):
        ...
        while iter <= config.max_iterations:
            ...
            if iter % 1000 == 0:
                ...
                if mle_avg < self.mle_avg_min and mle_avg<0.02:
                    print("Saving interim result...")
                    self.save_model(iter)
                    self.mle_avg_min=mle_avg
                count = mle_total = r_total = 0
